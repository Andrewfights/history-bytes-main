...
import {
  ...
  updatePearlHarborNodeMedia,
  fileToDataUrl,
  PearlHarborNodeMedia,
  PearlHarborMediaConfig,
...
interface NodeEditorProps {
  ...
  media: PearlHarborNodeMedia | null;
  onUpdate: (nodeId: string, media: Partial<PearlHarborNodeMedia>) => boolean;
  isExpanded: boolean;
  onToggle: () => void;
}
...
export function PearlHarborEditor() {
  const [mediaConfig, setMediaConfig] = useState<PearlHarborMediaConfig | null>(null);
  ...
  const handleNodeUpdate = (nodeId: string, media: Partial<PearlHarborNodeMedia>): boolean => {
  ...
  const expandAll = () => {
    setExpandedNodes(new Set(pearlHarborStory.nodes.map(n => n.id)));
  };
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-6 p-4 rounded-xl bg-card border border-border">
            <div>
              <p className="text-2xl font-bold">{pearlHarborStory.nodes.length}</p>
            ...
            <div className="h-10 w-px bg-border" />
            <div>
              <p className="text-2xl font-bold">{pearlHarborStory.totalXP}</p>
        ...
        <div className="mb-6 p-4 rounded-xl bg-red-500/5 border border-red-500/20">
          <h2 className="font-bold mb-2">{pearlHarborStory.title}</h2>
          <p className="text-sm text-muted-foreground mb-2">{pearlHarborStory.description}</p>
          <p className="text-xs text-red-400">
            Learning Arc: {pearlHarborStory.learningArc}
          </p>
        </div>

        {/* Node Editors */}
        <div className="space-y-4">
          {pearlHarborStory.nodes.map((node, index) => (
            <motion.div
              key={node.id}
