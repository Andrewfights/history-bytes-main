...
export function ExamQuestionRenderer({
  ...
  const renderQuestion = () => {
    switch (question.type) {
      case 'multiple-choice':
        return <MultipleChoiceExam question={question} onAnswer={onAnswer} />;

      case 'fill-in-blank':
        return <FillInTheBlank question={question} onAnswer={onAnswer} />;

      case 'branching-reveal':
        return <BranchingReveal question={question} onAnswer={onAnswer} />;

      case 'dual-slider':
        return <DualAnswerSlider question={question} onAnswer={onAnswer} />;

      case 'drag-timeline':
        return <DragTimeline question={question} onAnswer={onAnswer} />;

      case 'multi-select':
        return <MultiSelectQuestion question={question} onAnswer={onAnswer} />;

      case 'percentage-compare':
        return <PercentageComparison question={question} onAnswer={onAnswer} />;

      case 'two-part':
        return <TwoPartAnswer question={question} onAnswer={onAnswer} />;
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Host direction (PiP mode shows avatar) */}
